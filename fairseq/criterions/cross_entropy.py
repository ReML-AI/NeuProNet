...
class CrossEntropyCriterion(FairseqCriterion):
    def __init__(self, task, sentence_avg, class_weights, gamma, ovr_onehot):
        super().__init__(task)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if hasattr(model, 'auto_encoder') and model.auto_encoder:
        ...
        elif hasattr(model, 'sup_contrast') and model.sup_contrast:
            ...
            # encoder_out, graph_out, net_output = net_output
            loss, outputs = self.compute_loss(model, net_output, sample, reduce=reduce)
        sample_size = (
        ...
        if hasattr(model, 'sup_contrast') and model.sup_contrast:
